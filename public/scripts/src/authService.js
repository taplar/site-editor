var AuthService = new function() {
	...
	var buildApi = function() {
		...
		var functions = {
			executeCallbackIfCredentialsAreNotBlank: function( json, callback ) {
			...
			, logErrorIfUserIdOrPasswordIsUndefined: function( json ) {
		...
		var api = {
			...
			, actionSubmitLogin: function( event ) {
				...
				functions.logErrorIfUserIdOrPasswordIsUndefined( json );

				if ( keyService.isEnter( event ) ) {
					functions.executeCallbackIfCredentialsAreNotBlank( json, function() {
						ajaxService.POST({
							url: "?auth/validate"
							, input: json
							, fnSuccess: api.processLoginSubmit
			...
			, displayLogin: function() {
				ajaxService.GET({
					...
					, fnSuccess: api.processDisplayLogin
					, fnFailure: loggingService.unrecoverableError
				});
			}
			, logout: function() {
			...
			, processDisplayLogin: function( html ) {
				try {
					...
					$promptTemplate.find( "#password" ).keyup( api.actionSubmitLogin );
					$promptTemplate
						.find( "#userid" )
							.keyup( api.actionSubmitLogin )
							.focus();
				} catch ( error ) {
			...
			, processValidate: function( jsonString ) {
				try {
					...
					api.processResponseCode({
						responseCode: responseJson.responseCode
						, fnAuthorized: workspaceService.displayWorkspace
						, fnUnauthorized: api.displayLogin
					});
				} catch ( error ) {
					loggingService.unrecoverableError( error );
				}
